...
# --- Simülasyon Fonksiyonları (Gerçek projede bunlar veritabanı ve model çağrıları olacak) ---
...
def generate_spectrogram_figure(signal_id):
    ...
    for i in range(512):
        ...
        if 0 <= row_idx < 256:
            data[row_idx, i] = np.random.uniform(20, 40)
...
def run_analysis_pipeline(pipeline_type, steps):
    ...
    if pipeline_type == "Klasik ML (Random Forest, SVM)":
        metrics_data = {'Model': ['Random Forest', 'SVM'], 'Precision': [np.random.uniform(0.85, 0.95), np.random.uniform(0.80, 0.90)], 'Recall': [np.random.uniform(0.88, 0.98), np.random.uniform(0.82, 0.92)], 'F1-Score': [np.random.uniform(0.86, 0.96), np.random.uniform(0.81, 0.91)]}
    else:
        metrics_data = {'Model': ['CNN'], 'Precision': [np.random.uniform(0.92, 0.99)], 'Recall': [np.random.uniform(0.94, 0.99)], 'F1-Score': [np.random.uniform(0.93, 0.99)]}
    metrics_df = pd.DataFrame(metrics_data).round(3)
    cm = np.random.randint(0, 100, size=(2,2))
    cm_labels = ['Gürültü', 'Sinyal']
    cm_fig = go.Figure(data=go.Heatmap(z=cm, x=cm_labels, y=cm_labels, text=cm, texttemplate="%{text}", textfont={"size":20}, colorscale='Blues'))
...
with st.sidebar:
    # YENİ EKLENEN SATIR: Logo
    st.image("media/deepseti_logo.png", use_container_width=True)
...
if st.session_state.get('run_analysis', False):
    ...
    if not pipeline_steps_to_run:
        ...
        with tab3:
            ...
            bar_fig = go.Figure(data=[go.Bar(name='F1-Score', x=metrics_df['Model'], y=metrics_df['F1-Score'], text=metrics_df['F1-Score'], textposition='auto')])
